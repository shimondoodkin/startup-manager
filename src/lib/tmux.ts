...
export class Tmux {
  ...
  private run(args: string[]): Promise<{ ok: boolean; stdout: string; stderr: string }> {
    return new Promise((resolve) => {
      execFile(this.binary, args, { windowsHide: true, maxBuffer: 16 * 1024 * 1024 }, (error, stdout, stderr) => {
  ...
  sessionShell(): string | undefined {
    ...
    const dir = path.dirname(path.resolve(this.binary));
    return `${toCygwinPath(path.join(dir, 'bash.exe'))} --norc -i`;
  }
